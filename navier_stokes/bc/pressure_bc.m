function f = pressure_bc(f, side)
    ...
    if strcmp(side, 'east') % East outlet.   
        ...
        f(2:end-1, end, 4) = -f(2:end-1, end, 2) + (2*w_2*rho_boundary*(1+(((u_vect_w*c_2).^2)/(2*(cs^4)))-((u_vect_w.^2)/(2*(cs^2)))));
        f(2:end-1, end, 7) = -f(2:end-1, end, 6) + (2*w_6*rho_boundary*(1+(((u_vect_w*c_6).^2)/(2*(cs^4)))-((u_vect_w.^2)/(2*(cs^2)))));
        f(2:end-1, end, 8) = -f(2:end-1, end, 9) + (2*w_9*rho_boundary*(1+(((u_vect_w*c_9).^2)/(2*(cs^4)))-((u_vect_w.^2)/(2*(cs^2)))));
